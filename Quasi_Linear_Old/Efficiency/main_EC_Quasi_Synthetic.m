%%% ! Version: EC - Quasi Linear - Sythetic Data - Iterative method
clc
clear
...
v = rand(n,m);
v = v ./ sum(v, 2); %%% ! mimic the last funding support version
...
%%% * Box constraint 
%%% ! Do revision -> it is quasi linear not linear
% p_lower = max(v .* B ./ sum(abs(v))); - linear case -> there might be something wrong
% p_lower = max(v .* B ./ sum(abs(v),2)); -> right version
% p_upper = norm(B, 1) * ones(1, m); - linear case
...
%%% Testing
% 1. Test quasi_dual_solver
% disp('Testing quasi_dual_solver:');
...
%%% ! A good drawing way is very important
...
title('n=100, m=100', 'FontSize', 20); % Graph title with larger font size
