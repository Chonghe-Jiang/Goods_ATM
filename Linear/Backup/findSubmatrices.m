function result = findSubmatrices(A)
    ...
    result = {};
    processed_rows = [];
    %%% Todo: now the current version of column output is not right -> we just output the crossing
    %%% Todo: we should add the output to make sure it is okay to solve the equation
    %%% Todo: a possible way to achieve this: for each two elements
    while any(A(:))
