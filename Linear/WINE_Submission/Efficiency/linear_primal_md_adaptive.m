function [solution, time, iter, obj_values, distance_md] = linear_primal_md_adaptive(v, B, x_0, epsilon, max_iter, plot_flag, p_opt_solver, fval_solver, switch_step)
    ...
    while iter < max_iter
        ...
        if iter <= switch_step
            eta = 0.1; % Step size 4 for the first segment
        elseif iter <= 2 * switch_step
            eta = 0.2; % Step size 3 for the second segment
        elseif iter <= 3 * switch_step
            eta = 0.3; % Step size 2 for the third segment
        else
            eta = 0.4; % Step size 1 for the remaining iterations
        end
